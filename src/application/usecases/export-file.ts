...
export async function exportFile(secrets: EnvSecret[], filename: string = ".env") {
  const envString: [string, string?][] = secrets.map(secret => [secret.name, secret.value])

  const file = resolve(process.cwd(), filename)
  if (existsSync(file)) {
    const oldEnv = readFileSync(file).toString()
      .split("\n")
